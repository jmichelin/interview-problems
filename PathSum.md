...
### high level overview
    ...
    Starting at the root node, map path to each leaf summing values of each node to the leaf if the sum when reaching the leaf matches target sum return true otherwise return false
